...
signupSubmit.addEventListener('click', (e) => {
	...
	fetch(`/api/user`, {
		...
		if (result.ok) {
			...
			document.querySelector('.signup-message').style.backgroundColor = '#FDFDBD';
			document.getElementById('signup-container').style.height = '360px';
			check();
		} else {
			document.querySelector('.signup-message').style.display = 'block';
			document.querySelector('.signup-message').innerText = '註冊會員失敗,請提供其他E-mail';
...
loginSubmit.addEventListener('click', (e) => {
	...
	fetch('/api/user/auth', {
		...
		if (result.ok) {
			...
			document.querySelector('.login-message').style.backgroundColor = '#FDFDBD';
			document.querySelector('.login-message').style.color = '#FF6464';
			document.querySelector('.login-container').style.height = '300px';
			check();
		} else {
			document.querySelector('.login-message').style.display = 'block';
			document.querySelector('.login-message').innerText = '會員登入失敗,帳號或密碼錯誤';
			document.querySelector('.login-message').style.backgroundColor = '#FDFDBD';
...
function check() {
	fetch('/api/user/auth').then((response) => response.json()).then(function(result) {
